...
# --- MODIFIED: Sound functions are now disabled to prevent freezing ---
...
class Strategy:
    def __init__(self, params, manager: ConnectionManager, selected_index="SENSEX"):
        ...
        # Strategy Configuration & State Variables
    ...
    # --- Communication Methods (unchanged) ---
    ...
    async def _update_ui_chart_data(self):
        ...
        if self.current_candle.get("minute"):
            ...
            if not temp_df.empty: temp_df = pd.concat([temp_df, live_candle_df])
            else: temp_df = live_candle_df
    ...
    # --- Kite Ticker Event Handlers (unchanged) ---
    ...
    async def check_trade_entry(self):
        now = datetime.now()
        log_this_time = False
        if self.last_check_log_time is None or (now - self.last_check_log_time) > timedelta(seconds=10):
            log_this_time = True
            self.last_check_log_time = now
        
        if log_this_time: await self._log_debug("Check.Entry", "--- Running Entry Checks ---")
        
        if self.position:
            if log_this_time: await self._log_debug("Check.Entry", "-> FAIL: A position is already open.")
            return

        daily_sl, daily_pt = self.params.get("daily_sl", 0), self.params.get("daily_pt", 0)
        if self.daily_trade_limit_hit:
            if log_this_time: await self._log_debug("Check.Entry", "-> FAIL: Daily trade limit (SL/PT) has been hit.")
            return
        ...
        if self.trades_this_minute >= 2:
            if log_this_time: await self._log_debug("Check.Entry", f"-> FAIL: Trade frequency limit for this minute ({self.trades_this_minute}/2) reached.")
            return

        if self.data_df.empty or len(self.data_df) < 20:
            if log_this_time: await self._log_debug("Check.Entry", f"-> FAIL: Not enough historical data ({len(self.data_df)}/20 candles).")
            return
    ...
    async def take_trade(self, trigger, opt):
        ...
        play_entry_sound() # This call is now non-blocking
    ...
    async def evaluate_exit_logic(self):
        ...
        now = datetime.now()
        log_this_time = False
        if self.last_exit_log_time is None or (now - self.last_exit_log_time) > timedelta(seconds=5):
            log_this_time = True
            self.last_exit_log_time = now

        if log_this_time and not self.is_backtest:
            await self._log_debug("Check.Exit", f"LTP: {ltp:.2f} vs Trail SL: {p['trail_sl']:.2f}")
    ...
    async def check_uoa_entry(self, log=False):
        try:
            ...
            if current_second < 10 and not self.is_backtest:
                if log: await self._log_debug("Check.UOA", "-> FAIL: In confirmation period (0-10s).")
                return False
            for token, data in list(self.uoa_watchlist.items()):
                ...
                if self.aggressiveness == 'Conservative':
                    ...
                    if not trend_ok:
                        if log: await self._log_debug("Check.UOA", f"-> FAIL: Trend ({self.trend_state}) is against trade side ({side}).")
                        continue
                    ...
                    if not index_momentum_ok:
                        if log: await self._log_debug("Check.UOA", f"-> FAIL: Index momentum is against trade side ({side}).")
                        continue
                ...
                if not option_candle or 'open' not in option_candle or not current_price:
                    if log: await self._log_debug("Check.UOA", f"-> FAIL: Awaiting full candle data for {symbol}.")
                    continue
                if current_price <= option_candle['open']:
                    if log: await self._log_debug("Check.UOA", f"-> FAIL: Price ({current_price}) not above candle open ({option_candle['open']}).")
                    continue
                ...
                if not self.is_price_rising(symbol):
                    if log: await self._log_debug("Check.UOA", f"-> FAIL: Option price is not rising.")
                    continue
    ...
    async def check_ma_crossover_anticipation(self, log=False):
        try:
            ...
            if gap >= threshold:
                if log: await self._log_debug("Check.MA", f"-> FAIL: MA Gap ({gap:.2f}) is wider than threshold ({threshold:.2f}).")
                return False
            ...
            if sma > wma:
                if not self.is_price_rising(self.index_symbol):
                    if log: await self._log_debug("Check.MA", "-> FAIL: Index price not rising for bullish crossover.")
                    return False
            ...
            if wma > sma:
                if self.is_price_rising(self.index_symbol):
                    if log: await self._log_debug("Check.MA", "-> FAIL: Index price not falling for bearish crossover.")
                    return False
            ...
            if self.aggressiveness == 'Conservative':
                if self.trend_state == 'BULLISH':
                    if not (self.is_candle_bullish(self.index_symbol) and self.is_candle_bullish(opt['tradingsymbol']) and self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol'])):
                        if log: await self._log_debug("Check.Trend", "-> FAIL: Conservative bullish conditions not met.")
                        return False
                    ...
                    if not (not self.is_candle_bullish(self.index_symbol) and not self.is_candle_bullish(opt['tradingsymbol']) and not self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol'])):
                        if log: await self._log_debug("Check.Trend", "-> FAIL: Conservative bearish conditions not met.")
                        return False
                ...
                if self.trend_state == 'BULLISH':
                    if not (self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol'])):
                        if log: await self._log_debug("Check.Trend", "-> FAIL: Moderate bullish conditions not met.")
                        return False
                    ...
                    if not (not self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol'])):
                        if log: await self._log_debug("Check.Trend", "-> FAIL: Moderate bearish conditions not met.")
                        return False
        ...
        try:
            ...
            if last['rsi'] > last['rsi_sma'] and prev['rsi'] <= prev['rsi_sma']:
                ...
                if angle <= angle_thresh:
                    if log: await self._log_debug("Check.RSI", f"-> FAIL: Angle ({angle:.2f}) not steep enough ( > {angle_thresh:.2f}).")
                    return False
    ...
    async def add_to_watchlist(self, side, strike):
        ...
        if opt:
            ...
            await self._log_debug("UOA", f"Added {opt['tradingsymbol']} to watchlist."); await self._update_ui_uoa_list(); await play_entry_sound()
