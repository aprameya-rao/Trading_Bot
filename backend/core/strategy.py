...
class Strategy:
    ...
    async def handle_ticks_async(self, ticks):
        if self.ticker_manager and not self.initial_subscription_done and any(t.get('instrument_token') == self.index_token for t in ticks):
            ...
            tokens = self.get_all_option_tokens(); self.map_option_tokens(tokens); self.ticker_manager.resubscribe(tokens); self.initial_subscription_done = True
        for tick in ticks:
        ...
        if datetime.now().second % 2 == 0: await self._update_ui_status(); await self._update_ui_option_chain(); await self._update_ui_chart_data()
    ...
    def bootstrap_data(self):
        try:
            ...
            self._update_trend_state_sync()
            asyncio.run(self._log_debug("Bootstrap", f"Historical data loaded with {len(self.data_df)} candles."))
        except Exception as e:
            asyncio.run(self._log_debug("Bootstrap", f"Could not bootstrap data: {e}"))
            
    def _update_trend_state_sync(self):
        ...
        if self.trend_state != current_state:
            ...
            asyncio.run(self._log_debug("Trend", f"Trend is now {self.trend_state}."))
    ...
    async def check_trade_entry(self):
        ...
        # Call Strategies in Order of Priority
    ...
    async def check_uoa_entry(self, log=False):
        ...
        if datetime.now().second < 10: return False
        ...
        for token, data in list(self.uoa_watchlist.items()):
            ...
            option_candle = self.option_candles.get(symbol)
            ...
            if current_price > option_candle['open'] and self.is_price_rising(symbol):
                ...
                opt = self.get_entry_option(side, strike=data['strike'])
                await self.take_trade(trigger_reason, opt)
    ...
    async def check_trend_continuation(self, log=False):
        ...
        if self.aggressiveness == 'Conservative':
            if self.trend_state == 'BULLISH':
                opt = self.get_entry_option('CE')
                if opt and self.is_candle_bullish(self.index_symbol) and self.is_candle_bullish(opt['tradingsymbol']) and self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol']):
                    await self.take_trade('Trend_Continuation_CE', opt); return True
            if self.trend_state == 'BEARISH':
                opt = self.get_entry_option('PE')
                if opt and not self.is_candle_bullish(self.index_symbol) and not self.is_candle_bullish(opt['tradingsymbol']) and not self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol']):
                    await self.take_trade('Trend_Continuation_PE', opt); return True
        else:
            if self.trend_state == 'BULLISH':
                opt = self.get_entry_option('CE')
                if opt and self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol']):
                    await self.take_trade('Trend_Continuation_CE (M)', opt); return True
            if self.trend_state == 'BEARISH':
                opt = self.get_entry_option('PE')
                if opt and not self.is_price_rising(self.index_symbol) and self.is_price_rising(opt['tradingsymbol']):
                    await self.take_trade('Trend_Continuation_PE (M)', opt); return True
        return False
        
    def check_steep_reentry(self):
        last = self.data_df.iloc[-1]
        if last['wma'] > last['sma'] and not self.is_candle_bullish(self.index_symbol):
            self.pending_steep_signal = {'side': 'PE', 'reason': 'Reversal_PE'}
        if last['wma'] < last['sma'] and self.is_candle_bullish(self.index_symbol):
    ...
    async def check_pending_steep_signal(self):
        ...
        if opt and self.is_price_rising(opt['tradingsymbol']):
            await self.take_trade(f"{signal['reason']}", opt)
    ...
    async def check_rsi_immediate_entry(self, log=False):
        ...
        angle = self._calculate_rsi_angle()
        angle_thresh = self.STRATEGY_PARAMS['rsi_angle_threshold']
            ...
            try:
                def place_order_sync():
                    return kite.place_order(
                        ...
                        variety=kite.VARIETY_REGULAR, order_type=kite.ORDER_TYPE_MARKET,
                ...
                await self._log_debug("LIVE TRADE ERROR", f"Order placement failed: {e}"); play_loss_sound()
                return
        else:
    ...
    async def exit_position(self, reason):
            ...
            try:
                def place_order_sync():
                    return kite.place_order(
                        ...
                        transaction_type=kite.TRANSACTION_TYPE_SELL, quantity=p['qty'],
                        variety=kite.VARIETY_REGULAR, order_type=kite.ORDER_TYPE_MARKET,
                        product=kite.PRODUCT_MIS
                    )
                order_id = await asyncio.to_thread(place_order_sync)
                ...
                await self._log_debug("LIVE TRADE ERROR", f"Exit order placement failed: {e}"); play_loss_sound()
        else:
        ...
        await self._log_debug("Trade", f"Exited {p['symbol']} at {exit_price} for P&L: {net_pnl:.2f}. Reason: {reason}")
    ...
    def map_option_tokens(self, tokens):
        ...
        # This function might be called from a non-async context during bootstrap
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(self._log_debug("Tokens", f"Mapped {len(self.token_to_symbol)} symbols for websocket."))
        except RuntimeError: # No running loop
            asyncio.run(self._log_debug("Tokens", f"Mapped {len(self.token_to_symbol)} symbols for websocket."))
    ...
    def calculate_uoa_conviction_score(self, option_data, atm_strike):
        ...
        price_change_pct = option_data.get('change', 0)
        score += min(price_change_pct / 10.0, 5)
    ...
    async def scan_for_unusual_activity(self):
        ...
        try:
            ...
            await self._log_debug("Scanner", f"Error during intelligent UOA scan: {e}")