...
try:
    ...
    def generate_tone(frequency, duration_ms):
        sample_rate = 44100; t = np.linspace(0, duration_ms / 1000, int(sample_rate * duration_ms / 1000), False)
...
class Strategy:
    def __init__(self, params, manager: ConnectionManager, selected_index="SENSEX"):
        ...
        # --- FIX #1: Convert numeric string parameters to numbers ---
        try:
            numeric_params = ['start_capital', 'trailing_sl_points', 'trailing_sl_percent', 'daily_sl', 'daily_pt']
    ...
    # --- FIX #2: Add robust error handling to the UOA scanner ---
    async def scan_for_unusual_activity(self):
        try:
            ...
            # This will catch any error inside the scanner and log it, preventing a crash.
    ...
    async def _update_ui_trade_status(self):
        payload = None;
        if self.position: p = self.position; ltp = self.prices.get(p['symbol'], p['entry_price']); pnl = (ltp - p['entry_price']) * p['qty']; profit_pct = ((ltp - p['entry_price']) / p['entry_price']) * 100 if p['entry_price'] > 0 else 0; payload = {'symbol': p['symbol'], 'entry_price': p['entry_price'], 'pnl': pnl, 'profit_pct': profit_pct, 'trail_sl': p['trail_sl'], 'max_price': p['max_price']}
    ...
    async def _update_ui_chart_data(self):
        ...
        if self.current_candle.get('minute'): live_candle_df = pd.DataFrame([self.current_candle], index=[self.current_candle['minute']]); temp_df = pd.concat([temp_df, live_candle_df])
    ...
    async def bootstrap_data(self):
        try:
            ...
            df = pd.DataFrame(data).tail(100); df.index = pd.to_datetime(df['date']); self.data_df = self._calculate_indicators(df); await self._update_trend_state(); await self._log_debug("Bootstrap", f"Historical data loaded with {len(self.data_df)} candles.")
        except Exception as e: await self._log_debug("Bootstrap", f"Could not bootstrap data: {e}")
    ...
    async def update_candle_and_indicators(self, ltp, symbol=None):
        self.current_minute = datetime.now().replace(second=0, microsecond=0); is_index = (symbol is None or symbol == self.index_symbol)
        ...
        if candle_dict.get('minute') != self.current_minute:
            ...
            if is_index and 'minute' in candle_dict:
                new_row = pd.DataFrame([candle_dict], index=[candle_dict['minute']]); self.data_df = pd.concat([self.data_df, new_row]).tail(100); self.data_df = self._calculate_indicators(self.data_df); await self._update_trend_state()
    ...
    async def log_trade_decision(self, trade_info):
        def db_call():
            ...
            atr_value = self.data_df.iloc[-1]['atr'] if not self.data_df.empty and 'atr' in self.data_df.columns else 0; trade_info['atr'] = round(atr_value, 2)
            columns = ', '.join(trade_info.keys()); placeholders = ', '.join('?' * len(trade_info)); sql = f'INSERT INTO trades ({columns}) VALUES ({placeholders})'; cursor.execute(sql, tuple(trade_info.values())); conn.commit(); conn.close()
    ...
    async def check_trade_entry(self):
        ...
        if self.daily_trade_limit_hit:
            if datetime.now().second % 30 == 0: await self._log_debug("System", "Daily trade limit hit. No new trades."); return
        if daily_sl < 0 and self.daily_pnl <= daily_sl: self.daily_trade_limit_hit = True; play_warning_sound(); await self._log_debug("RISK", f"Daily Stop-Loss of {daily_sl} hit. Disabling trading."); return
        if daily_pt > 0 and self.daily_pnl >= daily_pt: self.daily_trade_limit_hit = True; play_profit_sound(); await self._log_debug("RISK", f"Daily Profit Target of {daily_pt} hit. Disabling trading."); return
        if self.trades_this_minute >= 2:
            if datetime.now().second % 15 == 0: await self._log_debug("Risk", "Max trades (2) for this minute reached."); return
