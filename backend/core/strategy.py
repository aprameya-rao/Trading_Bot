...
try:
    ...
    def play_sound(frequency, duration_ms):
        try:
            sa.play_buffer(generate_tone(frequency, duration_ms), 1, 2, 44100)
...
class Strategy:
    def __init__(self, params, manager: ConnectionManager, selected_index="SENSEX"):
        ...
        self.data_df = pd.DataFrame(
            columns=[
                "open", "high", "low", "close", "sma", "wma", "rsi", "rsi_sma", "atr"
            ]
        )
        ...
        try:
            ...
            self.STRATEGY_PARAMS = {
                "wma_period": 9, "sma_period": 9, "rsi_period": 9,
                "rsi_signal_period": 3, "rsi_angle_lookback": 2,
                "rsi_angle_threshold": 15.0, "atr_period": 14,
    ...
    async def _update_ui_status(self):
        payload = {
            "connection": "CONNECTED" if self.ticker_manager and self.ticker_manager.is_connected else "DISCONNECTED",
    ...
    async def _update_ui_performance(self):
        payload = {
            "netPnl": self.daily_pnl,
            "grossProfit": self.daily_profit,
            "grossLoss": self.daily_loss,
    ...
    async def _update_ui_trade_status(self):
        ...
        if self.position:
            p = self.position
            ltp = self.prices.get(p["symbol"], p["entry_price"])
    ...
    async def _update_ui_option_chain(self):
        ...
        if self.prices.get(self.index_symbol) and pairs:
            for p in pairs:
                ...
                data.append({
                    "strike": p["strike"],
                    "ce_ltp": self.prices.get(ce_symbol, "--") if ce_symbol else "--",
                    "pe_ltp": self.prices.get(pe_symbol, "--") if pe_symbol else "--",
                })
        ...
        if not temp_df.empty:
            for index, row in temp_df.iterrows():
                ...
                chart_data["candles"].append({
                    "time": timestamp, "open": row.get("open", 0),
    ...
    async def bootstrap_data(self, df: Optional[pd.DataFrame] = None):
        try:
            if df is None:
                ...
                df = pd.DataFrame(data).tail(300)
                df.index = pd.to_datetime(df["date"])

            self.data_df = self._calculate_indicators(df)
            await self._update_trend_state()
            await self._log_debug("Bootstrap", f"Historical data loaded. Bot is live with {len(self.data_df)} candles.")
        except Exception as e:
            await self._log_debug("Bootstrap", f"Could not bootstrap data: {e}")

    async def _update_trend_state(self):
        if len(self.data_df) < self.STRATEGY_PARAMS.get("sma_period", 9):
            return
        last = self.data_df.iloc[-1]
        if pd.isna(last["wma"]) or pd.isna(last["sma"]):
            return
    ...
    async def log_trade_decision(self, trade_info):
        ...
        def db_call():
            ...
            cursor.execute(sql, tuple(trade_info.values()))
            conn.commit()
            conn.close()

        await asyncio.to_thread(db_call)
        await self._log_debug("Database", f"Trade for {trade_info['symbol']} logged successfully.")
    ...
    async def check_trade_entry(self):
        ...
        if self.daily_trade_limit_hit:
            if datetime.now().second % 30 == 0 and not self.is_backtest:
                await self._log_debug("System", "Daily trade limit hit. No new trades.")
            return
        if daily_sl < 0 and self.daily_pnl <= daily_sl:
            self.daily_trade_limit_hit = True
            play_warning_sound()
            await self._log_debug("RISK", f"Daily Stop-Loss of {daily_sl} hit. Disabling trading.")
            return
        if daily_pt > 0 and self.daily_pnl >= daily_pt:
            self.daily_trade_limit_hit = True
            play_profit_sound()
            await self._log_debug("RISK", f"Daily Profit Target of {daily_pt} hit. Disabling trading.")
            return
        if self.trades_this_minute >= 2:
            if datetime.now().second % 15 == 0 and not self.is_backtest:
                await self._log_debug("Risk", "Max trades (2) for this minute reached.")
            return
        if self.position or self.data_df.empty or len(self.data_df) < 20:
            return
        
        # Volatility Filter
        last_atr = self.data_df.iloc[-1]["atr"]
        min_atr = self.STRATEGY_PARAMS["min_atr_value"] * (2 if self.index_name == "SENSEX" else 1)
        if pd.isna(last_atr) or last_atr < min_atr:
            return

        # Entry Strategy Checks
        log_this_minute = datetime.now().second % 15 == 0 and not self.is_backtest
        if await self.check_uoa_entry(log_this_minute): return
        if await self.check_ma_crossover_anticipation(log_this_minute): return
        if await self.check_trend_continuation(log_this_minute): return
        if self.check_steep_reentry(): return
        if await self.check_rsi_immediate_entry(log_this_minute): return

    # ... All check_* entry functions (unchanged from previous version but formatted) ...
    # ... take_trade, evaluate_exit_logic, exit_position (unchanged but formatted) ...
    ...
    async def take_trade(self, trigger, opt):
        ...
        if price is None or price < 1.0 or lot_size is None:
            await self._log_debug("Trade", f"Invalid price/lot_size for {symbol}: P={price}, L={lot_size}")
            return
        ...
        if risk_per_share <= 0:
            await self._log_debug("Risk", f"Cannot calculate quantity. Risk per share is zero or negative for {symbol}.")
            return
        ...
        if num_lots == 0:
            if capital > price * lot_size:
                num_lots = 1
                await self._log_debug("Risk", "Calculated lots is 0. Defaulting to 1 lot as capital permits.")
            else:
                await self._log_debug("Risk", f"Insufficient capital to take even 1 lot of {symbol}.")
                return
            ...
            try:
                def place_order_sync():
                    return kite.place_order(
                        tradingsymbol=symbol, exchange=self.exchange,
                        transaction_type=kite.TRANSACTION_TYPE_BUY,
                        quantity=qty, variety=kite.VARIETY_REGULAR,
                ...
                await self._log_debug("LIVE TRADE ERROR", f"Order placement failed: {e}")
                play_loss_sound()
                return
        else:
        ...
        self.position = {
            "symbol": symbol, "entry_price": price, "direction": side, "qty": qty,
            "trail_sl": round(initial_sl_price, 1), "max_price": price,
            "trigger_reason": trigger, "entry_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "lot_size": lot_size
        }
    ...
    async def evaluate_exit_logic(self):
        ...
        sl_percent = float(self.params["trailing_sl_percent"])
        p["trail_sl"] = round(
            max(p["trail_sl"], max(p["max_price"] - sl_points, p["max_price"] * (1 - sl_percent / 100))), 1
        )
        if not self.is_backtest:
            await self._update_ui_trade_status()
        if ltp <= p["trail_sl"]:
    ...
    async def exit_position(self, reason):
            ...
            try:
                def place_order_sync():
                    return kite.place_order(
                        ...
                        quantity=p["qty"], variety=kite.VARIETY_REGULAR,
                        order_type=kite.ORDER_TYPE_MARKET, product=kite.PRODUCT_MIS
                    )
                order_id = await asyncio.to_thread(place_order_sync)
                await self._log_debug("LIVE TRADE", f"Placed SELL order for {p['qty']} {p['symbol']}. Order ID: {order_id}")
            except Exception as e:
                await self._log_debug("LIVE TRADE ERROR", f"Exit order placement failed: {e}")
                play_loss_sound()
        else:
            await self._log_debug("PAPER TRADE", f"Simulating SELL order for {p['qty']} {p['symbol']}.")
        ...
        trade_entry = (
            p["symbol"], p["entry_time"], p["trigger_reason"],
            f"{p['entry_price']:.2f}", f"{exit_price:.2f}",
            f"{net_pnl:.2f}", reason,
        )
        self.trade_log.append(trade_entry)
        log_info = {
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "trigger_reason": p["trigger_reason"], "symbol": p["symbol"],
            "pnl": round(net_pnl, 2), "entry_price": p["entry_price"],
        ...
        self.position = None
        await self._update_ui_trade_status()
        await self._update_ui_trade_log()
    ...
    def _calculate_rsi_angle(self):
        ...
        if len(self.data_df) < lookback + 1:
            return 0
        rsi_values = self.data_df["rsi"].iloc[-(lookback + 1) :].values
        try:
            return math.degrees(math.atan(np.polyfit(np.arange(len(rsi_values)), rsi_values, 1)[0]))
        except (np.linalg.LinAlgError, ValueError):
            return 0
    # ... Rest of the file is identical to the previous 'final' version, just formatted.
    # It contains all the check_* functions, load_instruments, scan_for_unusual_activity etc.
    async def check_uoa_entry(self, log=False):
        try:
            ...
            if current_second < 10 and not self.is_backtest:
                if log and current_second % 3 == 0: await self._log_debug("UOA Check", "In confirmation period (0-10s). Waiting...")
                return False
            for token, data in list(self.uoa_watchlist.items()):
                symbol, side = data['symbol'], data['type']
                is_trade_valid = False; trigger_reason = "UOA_Entry_Unknown"
                if self.aggressiveness == 'Conservative':
                    ...
                    if not trend_ok:
                        if log: await self._log_debug("UOA Check", f"{symbol} failed. Trend is {self.trend_state}, required {'BULLISH' if side == 'CE' else 'BEARISH'}.")
                        continue
                    is_trade_valid = True; trigger_reason = "UOA_Trend_Confirmed"
                else:
                    ...
                    if not index_momentum_ok:
                        if log: await self._log_debug("UOA Check", f"{symbol} failed. Index momentum is against the trade.")
                        continue
                    is_trade_valid = True; trigger_reason = "UOA_Momentum_Confirmed"
                ...
                if not option_candle or 'open' not in option_candle or not current_price:
                    if log: await self._log_debug("UOA Check", f"Awaiting full candle data for {symbol}.")
                    continue
                if current_price <= option_candle['open']:
                    if log: await self._log_debug("UOA Check", f"{symbol} failed stability. Price ({current_price}) not above open ({option_candle['open']}).")
                    continue
                if self.is_price_rising(symbol):
                ...
                elif log: await self._log_debug("UOA Check", f"{symbol} failed final option momentum check.")
            return False
    ...
    def get_weekly_expiry(self):
        if self.is_backtest: return date.today() + timedelta(days=2) # Mock expiry
    ...
    def get_entry_option(self, side, strike=None):
        ...
        if self.is_backtest: # Mock instrument for backtesting
            return {'tradingsymbol': f"{self.index_name}{strike}{side}", 'instrument_type': side, 'strike': strike, 'lot_size': 15 if self.index_name == "SENSEX" else 25}
    ...
    async def add_to_watchlist(self, side, strike):
        ...
        if opt:
            # The token for a mocked instrument in backtest will be its symbol name
